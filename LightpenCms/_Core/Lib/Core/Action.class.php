...
abstract class Action {
   ...
   /**
     ...
     * @return void
     */
    protected function wapError($message,$jumpUrl='',$ajax=false) {
    	$this->wapDispatchJump($message,0,$jumpUrl,$ajax);
    }
     ...
     * @return void
     */
    protected function wapSuccess($message,$jumpUrl='',$ajax=false) {
    	$this->wapDispatchJump($message,1,$jumpUrl,$ajax);
    }
    /**
     * 默认跳转操作 支持错误导向和正确跳转
     ...
     * @return void
     */
    private function wapDispatchJump($message,$status=1,$jumpUrl='',$ajax=false) {
    	// 判断是否为AJAX返回
    	...
    	if($status) { //发送成功信息
    		...
    		$this->display(C('WAP_TMPL_ACTION_SUCCESS'));
    	}else{
    		...
    		$this->display(C('WAP_TMPL_ACTION_ERROR'));
    		// 中止执行  避免出错后继续执行
    		exit ;
    	}
    }

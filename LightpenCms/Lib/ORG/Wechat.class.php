...
class Wechat
{
    ...
    const SHAKEAROUND_ACCOUNT_REGISTER = '/shakearound/account/register?'; // 申请开通摇一摇周边
    // const SHAKEAROUND_ACCOUNT_AUDITSTATUS = '/shakearound/account/auditstatus?'; // 查询审核状态
    const SHAKEAROUND_DEVICE_GROUP_ADD = '/shakearound/device/group/add?'; // 新增分组(H5)
    const SHAKEAROUND_DEVICE_GROUP_UPDATE = '/shakearound/device/group/update?'; // 编辑分组(H5)
    const SHAKEAROUND_DEVICE_GROUP_DELETE = '/shakearound/device/group/delete?'; // 删除分组(H5)
    const SHAKEAROUND_DEVICE_GROUP_GETLIST = '/shakearound/device/group/getlist?'; // 查询分组列表(H5)
    const SHAKEAROUND_DEVICE_GROUP_GETDETAIL = '/shakearound/device/group/getdetail?'; // 查询分组详情(H5)
    const SHAKEAROUND_DEVICE_GROUP_ADDEVICE = '/shakearound/device/group/adddevice?'; // 添加设备到分组(H5)
    const SHAKEAROUND_DEVICE_GROUP_DELETEDEVICE = '/shakearound/device/group/deletedevice?'; // 从分组中移除设备(H5)
    // const SHAKEAROUND_DEVICE_GROUP_DELETEDEVICE = '/shakearound/device/group/deletedevice?'; // 从分组中移除设备(H5)
    const SHAKEAROUND_DEVICN_APPLYSTAUS = '/shakearound/device/applystatus?'; // 查询设备ID申请审核状态

    // 微信门店
    const CGIBIN_MEDIA_UPLOADIMG = '/cgi-bin/media/uploadimg?'; // 上传图片
    const CGIBIN_POI_ADDPOI = '/cgi-bin/poi/addpoi?'; // 创建门店
    const CGIBIN_POI_GETPOILIST = '/cgi-bin/poi/getpoilist?'; // 查询门店列表
    const CGIBIN_POI_GETPOI = '/cgi-bin/poi/getpoi?';  // 查询门店信息
    const CGIBIN_POI_UPDATEPOI = '/cgi-bin/poi/updatepoi?'; // 修改门店信息 
    const CGIBIN_POI_DELPOI = '/cgi-bin/poi/delpoi?'; // 删除门店
    const CGIBIN_API_GETWXCATEGORY = '/cgi-bin/api_getwxcategory?'; // 门店类目表
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function setCache($cachename,$value,$expired){
		...
		$data['expire_time'] = time() + $expired-300;
		$data['access_token'] = $value;
		$fp = fopen('./LightpenData/logs/Cache/access_token/'.$cachename.'.json', 'w');
	...
	protected function getCache($cachename=''){
	    ...
	    $data = json_decode(file_get_contents('./LightpenData/logs/Cache/access_token/'.$cachename.'.json'));
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		if ($result)
		{
			...
			$expire = $json['expires_in'] ? intval($json['expires_in']) : 0;
     ...
     * @param array $data 消息结构array('articles'=>array(array('thumb_media_id'=>'sdf451werwer15','author'=>'xxx','title'=>'Happy Day','content_source_url'=>'www.qq.com','content'=>'content','digest'=>'digest','show_cover_pic'=>'1')))
     * @return boolean|array
     */
    public function uploadGroupSendingNews($data)
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin/media/uploadnews?access_token=' . $this->access_token, $data);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     *           );
     */
    private function http_news_post($url,$param){
    	foreach ($param['articles'] as &$item){
    		foreach ($item as $k=>$v){
    			$item[$k] = htmlspecialchars_decode($v);
    			if($k=='content'){
    				$item[$k] = str_replace('="/Uploads/', '="'.C('site_url').'/Uploads/',$item[$k]);
    			}
    		}
    	}
    	$strPOST = self::json_encode($param);
    	$oCurl = curl_init();
    	if(stripos($url,"https://")!==FALSE){
    		curl_setopt($oCurl, CURLOPT_SSL_VERIFYPEER, FALSE);
    		curl_setopt($oCurl, CURLOPT_SSL_VERIFYHOST, false);
    	...
    	curl_setopt($oCurl, CURLOPT_POST,true);
    	curl_setopt($oCurl, CURLOPT_POSTFIELDS,$strPOST);
    	curl_setopt($oCurl, CURLOPT_HTTPHEADER, array("Content-Type: application/json; encoding=utf-8"));
    	$sContent = curl_exec($oCurl);
    	$aStatus = curl_getinfo($oCurl);
    	curl_close($oCurl);
    	if(intval($aStatus["http_code"])==200){
    ...
    /**
     *  上传多媒体文件
     * @param array $data 消息结构array('media'=>'/home/www/...jpg') 文件具体的路径
     * @param array $type 'image' 媒体文件类型，分别有图片（image）、语音（voice）、视频（video）和缩略图（thumb）s
     * @return boolean|array
     */
    public function uploadMediaFile($mediaPath, $type = 'image')
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data['media'] = '@' . $mediaPath;
        $result = $this->https_request('http://file.api.weixin.qq.com/cgi-bin/media/upload?access_token=' . $this->access_token . '&type=' . $type, $data);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    /**
     * post / get 请求
     * @param unknown $url 请求
     * @param string $data 数组数据 array('media'=>'@/home/www/...jpg');
     * @return mixed
     */
     ...
     * @return boolean|array
     */
	public function uploadForeverMedia($mediaPath, $type,$is_video=false,$video_info=array()){
        if (!$this->access_token && !$this->checkAuth()) return false;
      ...
      //  $data['media'] = '@' . $mediaPath;
        if(class_exists('\CURLFile')){
        	$data['media'] = new \CURLFile(realpath($mediaPath));
        }else{
        	$data['media'] = '@'.realpath($mediaPath);
        }
        //if ($is_video) $data['description'] = $video_info;
        if ($is_video) $data['description'] = '{"title":"'.$video_info['title'].'",  "introduction":"'.$video_info['introduction'].'"}';
//print_r($data);exit;
        $result = $this->https_request(self::API_URL_PREFIX.self::MEDIA_FOREVER_UPLOAD_URL.'access_token='.$this->access_token.'&type='.$type,$data);
        if ($result)
        {
        		//$json = $result;
           	$json = json_decode($result,true);
    ...
    /**
     * 创建卡券
     * @param string $data      卡券数据
     * @return array|boolean 返回数组中card_id为卡券ID
     */
    public function createCard($data) {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_CREATE . 'access_token=' . $this->access_token, $data);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @param string $data
     * 本来是数组直接转换为JSON使用的，但是转换为JSON 后quantity 会自动加上“”，导致不能直接申请，所以现在是直接在方法调用页面什么申请是组装的字符串来处理。
     * 例如：{"quantity":2,"apply_reason":"\u95e8\u5e97\u4f7f\u7528","comment":"test"}
     * @return boolean|mixed
     * {
        "data": {
            "apply_id": 123,
    ...
    public function applyShakeAroundDevice($data){
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::SHAKEAROUND_DEVICE_APPLYID . 'access_token=' . $this->access_token, $data);
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * [applyStatusShakearoundDevice 编辑设备的备注信息。可用设备ID或完整的UUID、Major、Minor指定设备，二者选其一。]
     * @param string $data
     * { "apply_id": 12345 }
     ...
     * "apply_time": 提交申请的时间戳 
     * "audit_comment": 审核状态。0：审核未通过、1：审核中、2：审核已通过；若单次申请的设备ID数量小于等于500个，系统会进行快速审核；若单次申请的设备ID数量大于500个，会在三个工作日内完成审核； 
     * "audit_status": 审核备注，对审核状态的文字说明 
     * "audit_time": 确定审核结果的时间戳，若状态为审核中，则该时间值为0 
     * @author binsee<binsee@163.com>
     * @version 2015-4-20 23:45:00
     */
    public function applyStatusShakearoundDevice($data){
        if (!$this->access_token && !$this->checkAuth()){
            return false;
        }
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::SHAKEAROUND_DEVICN_APPLYSTAUS . 'access_token=' . $this->access_token, $data);
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @param string $data
     * 本来是数组直接转换为JSON使用的，但是转换为JSON 后type,apply_id,begin,count 会自动加上“”，导致不能直接申请，所以现在是直接在方法调用页面什么申请是组装的字符串来处理。
     * '{"type": 3,"apply_id":1,"begin": 0,"count": 50}'
     * 
     * $data 三种格式:
     * ①查询指定设备时：$data = array(
    ...
    public function searchShakeAroundDevice($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::SHAKEAROUND_DEVICE_SEARCH . 'access_token=' . $this->access_token, $data);
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @copyright Show More
     */
    public function bindPageShakeAroundDevice($data,$bind=1,$append=1,$uuid='',$major=0,$minor=0){
        if (!$this->access_token && !$this->checkAuth()) return false;
        /* if(!$device_id){
            if(!$uuid || !$major || !$minor){
                return false;
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::SHAKEAROUND_DEVICE_BINDPAGE . 'access_token=' . $this->access_token, $data);
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * 上传在摇一摇页面展示的图片素材
     * 注意：数组的键值任意，但文件名前必须加@，使用单引号以避免本地路径斜杠被转义
     * @param array $data array('media'=>'@Path\filename.jpg') 格式限定为：jpg,jpeg,png,gif，图片大小建议120px*120 px，限制不超过200 px *200 px，图片需为正方形。
     * @return boolean|array
     * {
        "data": {
    ...
    public function uploadShakeAroundMedia($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post('https://api.weixin.qq.com/shakearound/material/add?access_token='.$this->access_token.'&type=icon',$data,true);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @copyright Show More
     */
    public function addShakeAroundPageNew($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::SHAKEAROUND_PAGE_ADD . 'access_token=' . $this->access_token, $data);
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
    ...
    /**
     * [updateShakeAroundPageNew 编辑摇一摇出来的页面信息，包括在摇一摇页面出现的主标题、副标题、图片和点击进去的超链接。]
     * @param string $str = '{"page_id":'.$page_id.',"title":"'.$data['maintitle'].'","description":"'.$data['nextitle'].'","icon_url":"'.$icon_url.'","page_url":"'.$data['url'].'"}';
     * @return boolean|mixed
     * 正确返回JSON 数据示例:
     * {
     "data": {
     ...
     * @copyright Show More
     */
    public function updateShakeAroundPageNew($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::SHAKEAROUND_PAGE_UPDATE . 'access_token=' . $this->access_token, $data);
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * 本来是数组直接转换为JSON使用的，但是转换为JSON 后page_ids 会自动加上“”，导致不能直接申请，所以现在是直接在方法调用页面什么申请是组装的字符串来处理。
     * { "page_ids":12345 }
     * @return boolean|mixed
     * 正确返回JSON 数据示例:
     * {
     ...
     * @copyright Show More
     */
    public function deleteShakeAroundPageNew($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::SHAKEAROUND_PAGE_DELETE . 'access_token=' . $this->access_token, $data);
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
    ...
    /**
     * [deviceShakeAroundStatisticsNew 以设备为维度的数据统计接口。
     * 查询单个设备进行摇周边操作的人数、次数，点击摇周边消息的人数、次数；查询的最长时间跨度为30天。]
     * {"device_identifier":{"device_id":1,"begin_date": 1,"end_date":1 }
     * 本来是数组直接转换为JSON使用的，但是转换为JSON 后device_id,begin_date,end_date 会自动加上“”，导致数据不合法，所以现在是直接在方法调用页面什么申请是组装的字符串来处理。
     * @param string $data = '{"device_identifier":{"device_id":'.$selectval.'},"begin_date": '.$beginTime.',"end_date": '.$endTime.' }';
     * @return boolean|mixed
     * 正确返回JSON 数据示例:
     * {
     "data": [
     ...
     * @copyright Show More
     */
    public function deviceShakeAroundStatisticsNew($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::SHAKEAROUND_STATISTICS_DEVICE . 'access_token=' . $this->access_token, $data);
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
    ...
    /**
     * [pageShakeAroundStatisticsNew 以页面为维度的数据统计接口。
     * 查询单个页面通过摇周边摇出来的人数、次数，点击摇周边页面的人数、次数；查询的最长时间跨度为30天。]
     * @param string $data  $data = '{"page_id":'.$selectval.',"begin_date": '.$beginTime.',"end_date": '.$endTime.' }';
     * @return boolean|mixed
     * 正确返回JSON 数据示例:
     * {
     "data": [
     ...
     * @version 2015-4-21 00:43:00
     */
    public function pageShakeAroundStatisticsNew($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::SHAKEAROUND_STATISTICS_DEVICE . 'access_token=' . $this->access_token, $data);
        $this->log($result);
        if ($result) {
     ...
     * 
     * 申请开通摇一摇周边
     * [registerShakeAroundAccound 申请开通摇一摇周边功能.
     * 成功提交申请请求后，工作人员会在三个工作日内完成审核。若审核不通过，可以重新提交申请请求。若是审核中，请耐心等待工作人员审核，在审核中状态不能再提交申请请求。]
     * @param array $data
     * array(
     *     "name" => "zhang_san",    // 联系人姓名，不超过20汉字或40个英文字母 
     *     "phone_number" => "13512345678",  // 联系人电话 
     *     "email" => "weixin123@qq.com",  // 联系人邮箱 
     *     "industry_id" => "0118",   // 平台定义的行业代号
     *     "qualification_cert_urls" => [   // 相关资质文件的图片url，图片需先上传至微信侧服务器，用“素材管理-上传图片素材”接口上传图片，返回的图片URL再配置在此处；当不需要资质文件时，数组内可以不填写url 
              "http://shp.qpic.cn/wx_shake_bus/0/1428565236d03d864b7f43db9ce34df5f720509d0e/0",
              "http://shp.qpic.cn/wx_shake_bus/0/1428565236d03d864b7f43db9ce34df5f720509d0e/0"
            ],
     *     "apply_reason" => "test" // 申请理由，不超过250汉字或500个英文字母(不是必须)
     * )
     * @return boolean|mixed
     * {
           "data": {
     ...
     * @since  2015-10-10
     */
    public function registerShakeAroundAccound($data){
        if (!$this->access_token && !$this->checkAuth()) return false;  // 没权限，退出
        // 调用接口并传入数组    // self::在本类中调用     // json_encode: php数组转换成json数组
        ...
        $this->log($result); // 保存日志
        if ($result) {  // 接口返回的值存在
            $json = json_decode($result, true);  // json_decode:json数组转换成php数组
            if (!$json || !empty($json['errcode'])) {   // 如果$json不存在或值为空（转化失败）  // empty():判断变量或表达式是否为空
                $this->errCode = $json['errcode'];  // 返回的错误代码
                $this->errMsg  = $json['errmsg'];   //
                return false;
            }
            return $json;
     ...
     * @since  2015-10-10
     */
    public function auditstatusShakeAroundAccount($data){
        /*****
            "apply_time": 1432026025,
            "audit_comment": "test",
            "audit_status": 1,
     ...
     * 
     * 新增分组
     * [addShakearoundDeviceGroup H５页面获取设备信息时,需先将设备进行分组管理。]
     * @param array $data
     * array(
     *      "group_name" => "test" // 分组名称，不超过100汉字或200个英文字母
     * )
     * @return boolean|mixed
     * {
           "data": {
     ...
     * @since  2015-10-10
     */
    public function addShakearoundDeviceGroup($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::SHAKEAROUND_DEVICE_GROUP_ADD . 'access_token=' . $this->access_token, self::json_encode($data));
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * 
     * 编辑分组
     * [updateShakearoundDeviceGroup 编辑设备分组信息，目前只能修改分组名。]
     * @param array $data
     * array(
     *     "group_id" => 123,  // 分组唯一标识，全局唯一 
     *     "group_name" => "test update"  // 分组名称，不超过100汉字或200个英文字母 
     * )
     * @return boolean|mixed
     * {
           "data": {
     ...
     * @since  2015-10-10
     */
    public function updateShakearoundDeviceGroup($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::SHAKEAROUND_DEVICE_GROUP_UPDATE . 'access_token=' . $this->access_token, self::json_encode($data));
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * 
     * 删除分组
     * [deleteShakearoundDeviceGroup 删除设备分组，若分组中还存在设备，则不能删除成功。需把设备移除以后，才能删除。]
     * @param array $data
     * array(
     *      "group_id" => 123  // 分组唯一标识，全局唯一 
     * )
     ...
     * @since  2015-10-10
     */
    public function deleteShakearoundDeviceGroup($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::SHAKEAROUND_DEVICE_GROUP_DELETE . 'access_token=' . $this->access_token, self::json_encode($data));
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * 
     * 查询分组列表
     * [getlistShakearoundDeviceGroup 查询账号下所有的分组]
     * 
     * @param unknown $data
     * array(
     *     "begin" => 0,   // 分组列表的起始索引值
     *     "count" => 10  // 待查询的分组数量，不能超过1000个 
     * )
     ...
     * @since  2015-10-10
     */
    public function getlistShakearoundDeviceGroup($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::SHAKEAROUND_DEVICE_GROUP_GETLIST . 'access_token=' . $this->access_token, self::json_encode($data));
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * 
     * 查询分组详情
     * [getdetailShakearoundDeviceGroup 查询分组详情，包括分组名，分组id，分组里的设备列表。]
     * @param unknown $data
     * array(
     *     "group_id" => 123, // 分组唯一标识，全局唯一 
     *     "begin" => 0, // 分组里设备的起始索引值
     *     "count" => 100  // 待查询的分组里设备的数量，不能超过1000个 
     * )
     * @return boolean|mixed
     * {
           "data": {
     ...
     * @since  2015-10-10
     */
    public function getdetailShakearoundDeviceGroup($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::SHAKEAROUND_DEVICE_GROUP_GETDETAIL . 'access_token=' . $this->access_token, self::json_encode($data));
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * 
     * 添加设备到分组
     * [adddeviceShakearoundDeviceGroup 添加设备到分组，每个分组能够持有的设备上限为10000，并且每次添加操作的添加上限为1000。只有在摇周边申请的设备才能添加到分组。]
     * @param unknown $data
     * array(
     *     "group_id" => 123,  // 分组唯一标识，全局唯一
     *     "device_identifiers" => [  // 设备id列表 
     *         "device_id":10100,   // 设备编号,若填了UUID、major、minor，即可不填设备编号，二者选其一
     *         "uuid":"FDA50693-A4E2-4FB1-AFCF-C6EB07647825",
     *         "major":10001,
     *         "minor":10002   
     *     ]
     ...
     * @since  2015-10-10
     */
    public function adddeviceShakearoundDeviceGroup($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::SHAKEAROUND_DEVICE_GROUP_ADDEVICE . 'access_token=' . $this->access_token, self::json_encode($data));
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * 
     * 从分组中移除设备
     * [deletedeviceShakearoundDeviceGroup 从分组中移除设备，每次删除操作的上限为1000。]
     * @param unknown $data
     * array(
     *     "group_id" => 123,  // 分组唯一标识，全局唯一
           "device_identifiers" => [ // 设备id列表
     ...
     * @since  2015-10-10
     */
    public function deletedeviceShakearoundDeviceGroup($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::SHAKEAROUND_DEVICE_GROUP_DELETEDEVICE . 'access_token=' . $this->access_token, self::json_encode($data));
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
    ...
    /**
     * 查询门店列表
     * [cgibinpoigetpoilist 批量查询自己名下的门店list，并获取已审核通过的poi_id。]
     * @param string $data
     * 本来是数组直接转换为JSON使用的，但是转换为JSON 后quantity 会自动加上“”，导致不能直接申请，所以现在是直接在方法调用页面什么申请是组装的字符串来处理。
     * 例如：{"quantity":2,"apply_reason":"\u95e8\u5e97\u4f7f\u7528","comment":"test"}
     * @return boolean|mixed
     ...
     * @since  2015-10-10
     */
    public function cgibinpoigetpoilist($data){
        if (!$this->access_token && !$this->checkAuth()){
            return false;
        }
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CGIBIN_POI_GETPOILIST . 'access_token=' . $this->access_token, $data);
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @since  2015-11-3
     */
    public function cgibinpoigetpoi($poiid){
        if (!$this->access_token && !$this->checkAuth()){
            return false;
        }
        $data = array(
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CGIBIN_POI_GETPOI . 'access_token=' . $this->access_token, self::json_encode($data));
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
    ...
    /**
     * 上传门店图片
     * [cgibinmediauploadimg 上传图片并获取url]
     * @param unknown $data
     ...
     * @since  2015-11-5
     */
    public function cgibinmediauploadimg($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin/media/uploadimg?access_token='.$this->access_token.'&type=icon',$data,true);
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @since  2015-11-5
     */
    public function cgibinpoiaddpoi($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CGIBIN_POI_ADDPOI . 'access_token=' . $this->access_token, $data);
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @since  2015-11-5
     */
    public function cgibinpoiupdatepoi($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CGIBIN_POI_UPDATEPOI . 'access_token=' . $this->access_token, $data);
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @since  2015-11-5
     */
    public function cgibinpoidelpoi($poiid){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data = array(
                'poi_id' => $poiid
        );
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CGIBIN_POI_DELPOI . 'access_token=' . $this->access_token, self::json_encode($data));
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @since  2015-11-5
     */
    public function cgibinapigetwxcategory(){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX . self::CGIBIN_API_GETWXCATEGORY . 'access_token='.$this->access_token);
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {

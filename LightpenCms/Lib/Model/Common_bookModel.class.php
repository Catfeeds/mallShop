<?php
class Common_bookModel extends Model{
	...
	public function getWhereCommonBookInfo($where){
		return M('common_book')->where($where)->field('id,sn,shopid,mid,bookdate,booktime,bookpeoplenum,bookcontact,bookcontactmoblie,bookgender,bookremark,bookstatus,createtime')->find();
	}
	/**
	 * 获得 订单 列表
	 * @return Ambigous <mixed, boolean, NULL, multitype:>
	...
	public function getWhereCommonBookList($where){
		return M('common_book')->where($where)->field('id,sn,shopid,mid,bookdate,booktime,bookpeoplenum,bookcontact,bookcontactmoblie,bookgender,bookremark,bookstatus,createtime')->order('id DESC')->select();
	}
}
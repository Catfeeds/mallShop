...
class AreplyAction extends UserAction{
	 ...
	 */
	public function set(){
		$this->makeTopUrl = $this->makeTopUrl_User(array($this->MANAGE_WELCOME,array('name'=>'我的API接入','url'=>U('Wechat/lists'),'rel'=>'','target'=>''),array('name'=>'微信自动回复设置','url'=>U('Other/set',array('wechatid'=>$this->_request('wechatid'))),'rel'=>'','target'=>''),array('name'=>'微信关注时回复','url'=>'','rel'=>'','target'=>'')));
		...
		$otherInfo = $this->areplyModel->where($otherWhere)->find();
		if(!$otherInfo){
			$otherWhere['id'] = guid();
			$otherWhere['updatetime'] = $otherWhere['createtime'] =time();
			$otherInfo = $this->areplyModel->find($this->areplyModel->add($otherWhere));
		}
	...
	public function subset(){
		if(IS_POST){
			...
			$result = M('areply')->where($otherWhere)->save($data);
			if($result){
				$info['code'] = '200';
				$info['msg'] = '设置成功';
			}else{

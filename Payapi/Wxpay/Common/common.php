<?php
    define('site_url', 'http://'.$_SERVER['SERVER_NAME']);//全局调用常量
...
/**
 * 【新】SCRM5商品ID、订单号、交易号、券号生成规则  移植为Base里面的方法
 * 商品ID：p+商品代表字母+公司号+5位定位流水 =》实物商品：g；积分商品：p；
 * 订单号：代表字母+生成日期时间+companyID+3位定位流水 =》 eshop：E、手机预订：B、门店收银：P、闪惠：W、外卖：P、手机点单：T、客房点单：H、代金券销售：V、团购券销售：G、计次卡销售：A、打赏：K、积分商城：J、增值业务：N、储值交易：S
 * 交易号：x+交易类型代表字母+生成日期时间+companyID+3位定位流水 =》微信支付 ：W、支付宝支付：A、现金支付：C、储值支付 ：S、 APPLEPAY支付：P、 银联借记卡支付：Y、VISA信用卡支付 ：V、万事达信用卡支付 ：M、积分获取 ：H、积分消耗 ：G、 储值充值：B
 ...
 * @since  2016-11-15
 */
function newOrderID($type, $prefix, $companyid){
    $time = time();
    if($type == 1){ //--------------------------- 商品id ---------------------------\\
    }elseif($type == 2){ //--------------------------- 订单号 ---------------------------\\
    }elseif($type == 3){ //--------------------------- 订交易号 ---------------------------\\
        ...
        $db = new db();
        $serialNumber = $db->table($actionName)->where(array('companyid'=>$companyid,'createtime'=>array('between',array($time,$time+1))))->count();

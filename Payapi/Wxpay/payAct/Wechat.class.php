...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function getCache($cachename=''){
		...
		/* $data = json_decode(file_get_contents('./LightpenData/logs/Cache/access_token/'.$cachename.'.json'));
		if ($data->expire_time < time()) {
		    if($data->refresh_token && $data->access_type == 3){
		        $componentAccessToken = file_get_contents(C('site_url').U('User/WechatTicket/getComponentAccessToken'));
		        if($componentAccessToken){
    		        $getAccessTokenUrl = 'https:// api.weixin.qq.com /cgi-bin/component/api_authorizer_token?component_access_token='.$componentAccessToken;
    		        $getAccessTokenData = json_encode(array('component_appid'=>'wx3377298b1b790dd4','authorizer_appid'=>$this->appid,'authorizer_refresh_token'=>$data->refresh_token));
    		        $getAccessTokenReturn = json_decode(http_post($getAccessTokenUrl, $getAccessTokenData),true);
    		        $getAccessTokenReturnDate['expire_time'] = time() + $getAccessTokenReturn['expires_in']-10;
    		        $getAccessTokenReturnDate['access_token'] = $getAccessTokenReturn['authorizer_access_token'];
    		        $getAccessTokenReturnDate['refresh_token'] = $getAccessTokenReturn['authorizer_refresh_token'];
    		        return $getAccessTokenReturnDate['access_token'];
    		        writeLog($getAccessTokenReturnDate, './LightpenData/logs/Cache/access_token/'.$cachename.'.json',FILE_USE_INCLUDE_PATH);
		        }else{
		            return '';
		        }
		    }else{
	 ...
	 * @param string $jsapi_ticket 手动指定jsapi_ticket，非必要情况不建议用
	 */
	public function getJsCardTicket($appid='',$jsapi_ticket_wxcard=''){
		if (!$this->access_token && !$this->checkAuth()) return false;
		if (!$appid) $appid = $this->appid;
		if ($jsapi_ticket_wxcard) { //手动指定token，优先使用
		    $this->jsapi_ticket_wxcard = $jsapi_ticket_wxcard;
		    return $this->jsapi_ticket;
		}
		$authname = 'wechat_jsapi_ticket_wxcard'.$appid;
		if ($rs = $this->getCache($authname))  {
			$this->jsapi_ticket_wxcard = $rs;
			return $rs;
		}
		...
		if ($result)
		{
			...
			$this->jsapi_ticket_wxcard = $json['ticket'];
			$expire = $json['expires_in'] ? intval($json['expires_in'])-100 : 3600;
			$this->setCache($authname,$this->jsapi_ticket_wxcard,$expire);
			return $this->jsapi_ticket_wxcard;
		}
		return false;
	}
	/**
    ...
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_news_post('https://api.weixin.qq.com/cgi-bin/media/uploadnews?access_token=' . $this->access_token, $data);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @copyright Show More
     */
    public function deleteShakeAroundPageD($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
     ...
     * @since  2015-10-10
     */
    public function deletedeviceShakearoundDeviceGroup(){
        if (!$this->access_token && !$this->checkAuth()) return false;

			            ...
			            session('viptime',$companyInfo['viptime']);
		              	...
		              	if($companyR){
			                ...
			                if($type == 'login'){
			                    ...
			                    $this->redirect(U('Index/register',array('oid'=>$openid,'type'=>'register')));
			                }
			            }
	            	}else{
	            		if($type == 'login'){
	            			$this->redirect(U('Index/register',array('oid'=>base64_encode($openid),'type'=>'login')));
	            		}else{
	            			$this->redirect(U('Index/register',array('oid'=>base64_encode($openid),'type'=>'register')));
	            		}
	            	}
				}else{
			    	$this->redirect(U('Index/index'));
	...
	public function register(){
		if($this->_get('oid')){
			if($this->_get('type') == 'login'){
				...
				$this->redirect(U('Index/emailRegister',array('oid'=>base64_encode($this->_get('oid')))));
			}
			$openid = $this->_get('oid');
			$this->assign('status',$status);
			$this->assign('oid',$openid);
			$this->display();
		}else{
			$this->display();
		}
	...
	public function emailRegister(){
		if(IS_POST){
			...
			if($users){
				...
				$var = array("to" => array($to),"sub" => array("%register_url%" => Array(C('site_url').'/index.php?m=Index&a=registerOk&code='.base64_encode($data['code']).'&email='.base64_encode($to).'&oid='.$oid)));   //模板替换变量
...
//判断手机重复
        ...
        if($tell){
        	...
        	if($registerCount){  // 有未使用的手机号
        		...
        		if($registerCount1['useendtime'] >= time()){  // 未过期
        			$sendData = $this->sendSms($tell,$registerCount1['code'].'（注册验证码，请完成验证），如非本人操作，请忽略本短信。');
        			...
        			if($list) {
        				...
        				if ($insterData1){
        					$sendData = $this->sendSms($tell,$code['code'].'（注册验证码，请完成验证），如非本人操作，请忽略本短信。');
        					if($sendData['code'] == 200){
        						$data['code'] = '200';
        						$data['tips'] ='验证码短信已发送到你的手机上，有效时间为30分钟。';
        					}
	...
	public function registerOk(){
		if(IS_POST){
			...
			if(IS_GET){
				...
				if($emailInfo){
					$this->error('该邮箱已经注册请登录',U('Index/index'));
				}else{
					$oid = base64_decode($this->_get('oid'));
	...
	public function findPassword(){
		if(IS_POST){
			...
			if($tellInfo){
				...
				$sendData = $this->sendSms($tell,'尊敬的'.$user.',您的登录密码为：'.$tellInfo['truePassword'].'请勿泄露，如非本人操作，请忽略本短信。');
			}else{
				$data['status'] = '0';   //未注册
			}
		echo json_encode($data);
	...
	//合作代理页面
	public function cooptive(){
		$this->assign('title','合作代理');
		$this->display();
	}
	//帮助文档详细页
	public function detailed(){
		$this->assign('title','帮助文档');
		$this->display();
	}
	//常见问题页
	...
	//公告详细页
	public function anmdetails(){
		$this->assign('title','公告详细');
		$this->display();
	}
	//投资者关系
	public function investor(){
		$this->assign('title','投资者关系');
		$this->display();
	}
	//客户案例
	...
	//案例
	public function index3_case(){
		if($this->_get('type')){
			$id = M('hcms_class')->where(array('parentid'=>$this->_get('type'),'type'=>'3'))->getField('id',true);
			$idAll = implode(',', $id).','.$this->_get('type');
			$newCase = M()->table('tp_hcms_article as ha')->join('left join tp_hcms_class as hc on ha.classid = hc.id')->order('ha.sort')->limit(6)->where(array('hc.type'=>'3','ha.classid'=>array('in',$idAll)))->field('ha.id,ha.title,ha.img1,ha.updatetime')->select();
		}else{
			$newCase = M()->table('tp_hcms_article as ha')->join('left join tp_hcms_class as hc on ha.classid = hc.id')->order('ha.sort')->limit(6)->where(array('hc.type'=>'3'))->field('ha.id,ha.title,ha.img1,ha.updatetime')->select();
		}
	...
	//案例详情
	public function index3_casedetails(){
		if($this->_get('id')){
			...
			if($caseInfo && $newCase){
				$data = $this->makeTopUrl(array(array('name'=>'案例','url'=>U('Index/index3_case'),'rel'=>'','target'=>''),array('name'=>$caseInfo['title'],'url'=>U('Index/index3_case'),'rel'=>'','target'=>'')));
	...
	//公告详情
	public function index3_noticedetails(){
		if($this->_get('id')){
			...
			$data = $this->makeTopUrl(array(array('name'=>'公告','url'=>U('Index/index3_notice'),'rel'=>'','target'=>''),array('name'=>$noticeInfo['title'],'url'=>U('Index/index3_notice'),'rel'=>'','target'=>'')));
			$this->assign('data',$data);
	...
	//公告
	public function index3_notice(){
		$data = $this->makeTopUrl(array(array('name'=>'公告','url'=>U('Index/index3_notice'),'rel'=>'','target'=>'')));
	...
	//关于人来风
	public function index3_aboutus(){
		$this->assign('title','关于人来风');
		$this->display();
	}
	//法律声明
	public function index3_lawstatement(){
		$this->assign('title','法律声明');
		$this->display();
	}
	//摇摇魔石
	public function index3_yymoshi(){
		$this->assign('title','摇摇魔石');
		$this->display();
	}
	//DMS
	public function index3_dms(){
		if(IS_POST){
	...
	//现场互动墙
	public function index3_hdq(){
		$this->assign('title','现场互动墙');
		$this->display();
	}
	//手机收款
	public function index3_phonesk(){
		$this->assign('title','手机收款');
		$this->display();
	}
	//帮助中心
	public function index3_helpcenter(){
		if($this->_get('id')){
	...
	//注册
	public function index3_register(){
		$this->assign('title','注册');
		$this->display();
	}
	//支付方式
	public function index3_payment(){
		$helpClass = M('hcms_class')->order('classsort')->where(array('type'=>'1','parentid'=>'0'))->field('id,name')->select();
		foreach($helpClass as $key=>$val){
			$helpTitle = M('hcms_article')->order('sort')->where(array('type'=>'1','classid'=>$val['id']))->field('id,title')->select();
			$helpClass[$key]['son'] = $helpTitle;
	...
	public function makeTopUrl($now_path = array()){
		$return_url = '<a href="'.U('Index/index3').'" title="人来风帐户管理">首页</a><em>&gt;</em>';
		if (isset($now_path)){

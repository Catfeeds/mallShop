...
if(!isset($_SESSION)){
	...
	function usememcache($mem){
    	global $judge,$companyid;
		require ('db.php');
    	include('../wall/biaoqing.php');
    		$sql = 'SELECT * FROM  `tp_wall_shake_toshake` WHERE `companyid`='.$companyid;
            ...
            if( !$data ){
   			...
   			$start=realpath("..")."UPDATE  `tp_wall_wall_config` SET  `isopen` = '' where `companyid` = $companyid";
        	...
        	if($judge == 1){
				...
				$startvalue = 1;
                $mem->set($key2, $startvalue, MEMCACHE_COMPRESSED, 3600);
                mysql_query("update `tp_wall_shake_toshake` set `point`='0' where `companyid` = $companyid");
         	}
		...
		function usemysql(){
        	global $judge,$companyid;
         	...
         	$sql1="SELECT * FROM  `tp_wall_shake_toshake` WHERE `companyid`=".$companyid." order by point desc";
        ...
        //var_dump( $arr_one[0]['phone']);
         if($judge == 1){
        	...
        	$num=count($arr_one);
         	...
         	$start="UPDATE  `tp_wall_wall_config` SET  `isopen` =  '2' WHERE `companyid` = ".$companyid;
        	$query1=mysql_query($start,$link) or die(mysql_error());
         }else if($judge == 4){
         	
         	/* echo $sql = "UPDATE `tp_wall_shake_toshake` SET `point`='0' WHERE `companyid` = ".$companyid;
         	$query2=mysql_query($sql,$link);
         	var_dump($query2); */
         	
         	$start="UPDATE  `tp_wall_wall_config` SET  `isopen` =  '1' WHERE `companyid` = ".$companyid;
        	$query1=mysql_query($start,$link) or die(mysql_error());
 		}
	}

...
<script>
var votecannum=<?php echo $voteInfo['votecannum'];?>;
var hasvoted=0;
</script>
...
<div class="column">
		...
		<h2 class="ui top attached green segment votehead">
		   <?php echo $voteInfo['votetitle'];?><small>(请选择<red><?php echo $voteInfo['votecannum'];?></red>个选项)</small>
		</h2>
		<div class="ui attached segment content">
		<form method='post' action="function.php?do=vote&companyid=<?php echo $companyid;?>">
		...
		$vote_check = mysql_fetch_row($query_vote_check);
		if($vote_check <= 0 ){
			die("没有查询到您的信息，请返回微信，重新签到并投票！");
			//echo "<script>window.location='luru.php';</script>";
		}
		$class=array('','red','blue','green');
		$sql1="select sum(`res`)  from `tp_wall_vote` where `companyid`=".$companyid;
		$query1=mysql_query($sql1,$link) or die(mysql_error());
		$sum=mysql_fetch_row($query1);
		$sum=$sum[0];
		if($sum == 0){$sum = 1;}
			$sql="SELECT * FROM  `tp_wall_vote` where `companyid`=".$companyid." order by res desc";
...
<?php }}?>
		...
		<!--  <div class="footer">
			<center><a href="<?php echo site_url;?>"><img src="images/logo_vote.png" width="300"/></a></center>
		</div>-->
 </div>
</body>
</html>

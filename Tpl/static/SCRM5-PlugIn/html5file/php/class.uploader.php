...
class Uploader {
     ...
     * @return array
     */
    public function upload($field, $options = null,$type=1){
        $this->data = $this->cache_data;
        return $this->initialize($field, $options, $type);
    }
     ...
     * @return array
     */
    private function initialize($field, $options, $type){
		if(is_array($field) && in_array($field, $_FILES)){
			$oldname = $field['name']['0'];
			$temp_arr = explode(".", $oldname);
			$file_ext = array_pop($temp_arr);
			$file_ext = trim($file_ext);
			$file_ext = strtolower($file_ext);
			if($type==1){
				$field['name']['0'] = date("YmdHis") . '_' . rand(10000, 99999) . '.' . $file_ext;
			}else{
		...
		return $this->prepareFiles($oldname);
    }
     ...
     * @return $this->data
     */
	private function prepareFiles($oldname){
        $field = $this->field;
        ...
        if($validate){
            ...
            for($i = 0; $i < count($field['name']); $i++){
                ...
                //$metas['old_name'] = substr($oldname, 0, -(strlen(strrchr($oldname, $metas['extension']))+1));
                $metas['old_name'] = substr($oldname, 0, strripos($oldname, '.'));
                $metas['size'] = $field['size'][$i];
                $metas['size2'] = $this->formatSize($metas['size']);
                //$metas['name'] = $this->generateFileName($this->options['title'], array('name'=>substr($field['name'][$i], 0, -(strlen(strrchr($field['name'][$i], $metas['extension']))+1)), 'size'=>$metas['size'])) . (!empty($metas['extension']) ? "." . $metas['extension'] : "");
                $metas['name'] = $this->generateFileName($this->options['title'], array('name'=>substr($field['name'][$i], 0, strripos($field['name'][$i],'.')), 'size'=>$metas['size'])) . (!empty($metas['extension']) ? "." . $metas['extension'] : "");
                $metas['file'] = substr($this->options['uploadDir'] . $metas['name'],'14');
                $fieldurl = $this->options['uploadDir'] . $metas['name'];
                $metas['replaced'] = file_exists($fieldurl);
                $metas['date'] = date('r');
                
                if(!in_array($field['name'][$i], $removed_files) && $this->validate(array_merge($metas, array('index'=>$i, 'tmp'=>$tmp_name))) && $this->uploadFile($tmp_name, $fieldurl)){
                    if($this->options['perms']) @chmod($fieldurl, $this->options['perms']);

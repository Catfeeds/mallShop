		...
		function createView(result) {
			...
			for (var i = 0, len = fileList.length; i < len; i++) {
				...
				if(data.is_photo){//如果是图片
				    var _span=K('<span class="xl_span" data-url="'+K.formatUrl(result.current_url + data.filename, 'absolute')+'" style="position:absolute;display:none;width:102px;background:#0690d2;color:#FFF;text-align:center; cursor: pointer;line-height:20px;bottom:-4px;left:-20px;">删除</span>');
				    div.append(_span);
				}
			}
			K(".xl_span").click(function(){//这里的.xl_span对应着刚才插入的删除按钮上的class
			    ...
			    $.post('/Tpl/static/kindeditor/php/file_manager_json.php',{action:"delete",url:$this.attr("data-url")},function(res){//jquery的post，action为指定的配合后端用，url是获取刚才存在删除按钮上的图片路径，你完全可以用别的来写，因为用不好kindeditor的js库才用的jquery
...
KindEditor.plugin('multiimage', function(K) {
	...
	self.plugin.multiImageDialog = function(options) {
		...
		var swfupload = K.swfupload({
			...
			buttonImageUrl : imgPath + (self.langType == 'zh' ? 'select-files-zh_CN.png' : 'select-files-en.png'),
			buttonWidth : self.langType == 'zh' ? 72 : 88,
			buttonHeight : 23,
